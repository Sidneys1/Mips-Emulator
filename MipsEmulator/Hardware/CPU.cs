...
namespace MipsEmulator.Hardware
{
	internal class CPU : INotifyPropertyChanged
	{
		...
		uint _nextIP = Constants.PcAdvance;
		public uint NextIP
		{
			get { return _nextIP; }
			set
		...
		public CPU()
		{
			...
			Registers[(uint)Register.TrapStatus].Value = (uint)(InterruptStatus.SysCall | InterruptStatus.Ofv | InterruptStatus.Int | InterruptStatus.Ibus);
		}
		...
		public void Tick()
		{
			...
			MemoryItem memEntry = MainMemory[InstructionPointer / 4];

			var opcode = (Opcodes)memEntry.Opcode;
			switch (opcode)
			{
				case 0x0:
					Functcodes funct = (Functcodes)memEntry.Funct;
					switch (funct)
					{
						case Functcodes.Add:
						...
						default:
							throw new NotImplementedException(string.Format("Opcode {0} Funct {1} not implemented!", (int)opcode, (int)memEntry.Funct));
					}
					break;
				...
				default:
					throw new NotImplementedException(string.Format("Opcode {0} not implemented!", opcode));
			}
		}
